...
class SystemEventLogger:
    ...
    Captures execution logs, indicator calculations, risk guard checks, and
    order lifecycle events.

    Buffer size: 500 entries (increased from 150 to handle 3-symbol engines
    emitting signals every minute without premature eviction).
    """

    _instance = None
    _lock = threading.Lock()

    def __new__(cls, maxlen: int = 500):
        with cls._lock:
            if cls._instance is None:
                ...
                cls._instance.log(
                    "SYSTEM",
                    "Motor A3 Enterprise HFT iniciado y listo para recibir órdenes",
                    symbol="SYSTEM",
                    level="SUCCESS",
                )
